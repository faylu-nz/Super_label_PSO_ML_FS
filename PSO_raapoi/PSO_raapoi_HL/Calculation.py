...
def fullstd_PSOsel_std_simple_f1(datasets_list, no_clses_list, run, outDir, seed):
    ...
    for datasets, no_cls in zip(datasets_list, no_clses_list):
        for i in range(len(datasets)):
            ...
            # f = open('records/full_PSOsel_std_sim_f1/record_' + datasets[i] + str(run) +
            #          '_fullstd_PSOsel_std_simple_f1.txt', 'w')
            f = open(outDir + datasets[i] + '_' + str(run) +
                     '_fullstd_PSOsel_std_simple_f1.txt', 'w')
            f.write(to_print)
            f.close()


def fullstd_PSOsel_std_simple_hl(dataset, no_cls, run, outDir, seed):
    ...
    for train_idx, test_idx in k_fold.split(X, y):
        ...
        to_print += 'Fold selected standard PSO hamming loss: %.4f\n' % sel_std_ham
        to_print += 'Fold selected super simple PSO hamming loss: %.4f\n' % sel_sim_ham
    ...
    to_print += '--------------Average----------------\n'
    to_print += 'Ave Full Accuracy hamming loss: %.4f\n' % np.average(
        full_hams)
    to_print += 'Ave Standard PSO Selection Accuracy hamming loss: %.4f\n' % np.average(
        sel_std_hams)
    to_print += 'Ave Super simple PSO Selection Accuracy  hamming loss: %.4f\n' % np.average(
        sel_sim_hams)
    ...
    f = open(outDir + dataset + '_' + str(run) +
                '_fullstd_PSOsel_std_simple_f1.txt', 'w')
    #f = open(str(run) + datasets[i] + '_fullstd_PSOsel_std_sim_hl.txt', 'w')
    f.write(to_print)
    f.close()
